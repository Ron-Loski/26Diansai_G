...
#define HMI_SPECTRUM_BAR_HALF_W  2U
#define HMI_SPECTRUM_EDGE_TOL_HZ 1000.0f
...
static void HMI_DrawSpectrum(const Analyzer_Result_t *result)
{
  ...
  for (i = 0U; i < result->component_count; ++i)
  {
    ...
    uint16_t drawable_width =
        HMI_SPECTRUM_W - 3U - 2U * HMI_SPECTRUM_BAR_HALF_W;
    uint16_t x;
    uint16_t top;
    ...
    if ((frequency < 0.0f) ||
        (frequency >
         HMI_SPECTRUM_MAX_HZ + HMI_SPECTRUM_EDGE_TOL_HZ))
    {
      continue;
    }
    if (frequency > HMI_SPECTRUM_MAX_HZ)
    {
      frequency = HMI_SPECTRUM_MAX_HZ;
    }

    x = (uint16_t)(HMI_SPECTRUM_X +
                   1U +
                   HMI_SPECTRUM_BAR_HALF_W +
                   frequency * drawable_width /
                   HMI_SPECTRUM_MAX_HZ);
    top = (uint16_t)(bottom -
                     (HMI_SPECTRUM_H - 28U) *
                        ...
                        uint32_t wave_count)
{
  ...
  for (i = 0U; i < ANALYZER_MAX_COMPONENTS; ++i)
  {
    ...
    if (i < result->component_count)
    {
      ...
      HMI_DrawText(884U, y, 100U, 28U, HMI_FONT_UI, HMI_COLOR_FREQ,
